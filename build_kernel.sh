...
if [ -e $CONFIG/cust_defconfig ]; then
...
elif [ -e $CONFIG/sanders-perf_defconfig ]; then
# Device Name and Model
MODEL=Motorola G5S Plus
DEVICE=Sanders
# Kernel Version Code
VERSION=X3
# Kernel Defconfig
DEFCONFIG=sanders-perf_defconfig
# Linker
LINKER=ld.lld
# Path
DTBTOOL=$(pwd)/Dtbtool/
...
function clone() {
    # Clone Toolchain
		...
		post_msg " Cloning Arter GCC ToolChain "
		...
		elif [ $DEVICE = "Sanders" ]; then
		git clone https://github.com/reaPeR1010/AnyKernel3.git -b SANDERS AnyKernel3
		fi
}
##------------------------------------------------------##
...
function compile() {
    ...
    post_msg " Generating Master DTB "
